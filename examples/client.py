...
class Client(object):
    ...
    def _request(self, method, path, signed, **kwargs):
        ...
        tmp_session=self._init_session()
        
        response = getattr(tmp_session, method)(uri, **kwargs)
    ...
    def _handle_response(self, response):
        ...
        if not str(response.status_code).startswith('2'):
            print("handle_response status_code:"+str(response.status_code))
            raise BigoneAPIException(response)
        try:
            json = response.json()
            #print('handle_res,json:',json)
    ...
    def _get(self, path, signed=False, **kwargs):
        return self._request('get', path, signed)
    ...
    def get_accounts_fb(self,output):
        output.put(self._get('accounts', True))
        return
    ...
    def get_order_book_fb(self, symbol,output):

        output.put(self._get('markets/{}/book'.format(symbol), True))
        return
    ...
    def create_order(self, symbol, side, price, amount):
        ...
        print("create order para:",data)

        re_create_order=self._post('orders', True, data=data)

        print("order:",symbol," re:",re_create_order)

        return re_create_order
